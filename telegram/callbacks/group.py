...
async def __new_group_photo_message(sender: CrmUser, reciever: CrmUser, group: Group, caption: str, photos: list[str], forward_to: CrmUser = None):
    ...
    try:
        if forward_tuser:
            caption = i18n.gettext("new_forward_group_document_message", locale=forward_tuser.language).format(
                group=group.title,
                sender=" ".join((sender.first_name, sender.last_name)),
                caption=caption
            )
        ...
        album = []
        for i, document in enumerate(photos[:10]):
...
async def __new_group_document_message(sender: CrmUser, reciever: CrmUser, group: Group, caption: str, documents: list[str], forward_to: CrmUser = None):
    forward_tuser: User = (await db.ex(dmth.GetOne(User, id=forward_to.user_id))) if forward_to else None
